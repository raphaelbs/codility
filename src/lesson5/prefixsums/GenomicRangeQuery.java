	...
	public static void main(String[] args) {
		Log.run(new IExecuter() {
			...
			public Object execute() {
				return solution("ATTAATTAATTAATATAT",
						new int[] { 0, 2, 4, 6, 8 },
						new int[] { 1, 3, 5, 7, 9 });
			}
		});
...
class TreeManager { 
    ...
    private boolean containAny(final Node currentNode, final int P, final int Q) {
    	...
    	if (currentNode.data > P && currentNode.data < Q) return true;
    	if (currentNode.data > Q) return containAny(currentNode.left, P, Q);
    	if (currentNode.data < P) return containAny(currentNode.right, P, Q);
    	return false;
    }
}
